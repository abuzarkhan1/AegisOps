...
export function ReleasesPage() {
  ...
  async function load() {
    ...
    setError("");
    try {
      const [deploymentRows, serviceRows, incidentRows] = await Promise.all([
        fetchDeployments(),
        fetchServices(),
        fetchIncidents()
      ]);
    ...
    <div className="grid gap-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">Deployment stream, latest service versions, and impact analysis.</p>
        </div>
      ...
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <StatCard label="Latest version" value={selectedDeployment?.version ?? selectedDeployment?.commitSha?.slice(0, 7) ?? "-"} detail={selectedDeployment?.serviceName ?? "none selected"} />
      </div>
          ...
          <div className="grid gap-2">
            {deployments.map((deployment) => (
              <button
                ...
                className={`rounded-lg border p-3 text-left transition ${selectedDeploymentId === deployment.id ? "border-mint bg-mint/10" : "border-line bg-panel-soft hover:border-slate-500"}`}
              >
                ...
                <p className="mt-1 text-xs text-slate-400">{deployment.version ?? deployment.commitSha ?? "unversioned"}</p>
                <p className="mt-1 text-xs text-slate-500">{formatDate(deployment.createdAt)}</p>
              </button>
            ))}
            {deployments.length === 0 ? <p className="text-sm text-slate-500">No deployments found for the selected environment.</p> : null}
          </div>
        </Card>
        ...
        <div className="grid gap-5">
          <Card title="Selected Release" description="Deployment details and impact metadata.">
            {selectedDeployment ? (
              ...
              <p className="text-sm text-slate-500">Select a deployment to inspect release details.</p>
            )}
          </Card>
          ...
          <Card title="Service Release State" description="Latest deployment per service.">
            <div className="grid gap-3 xl:grid-cols-2">
              {latestByService.map((deployment) => (
                <div key={`${deployment.serviceName}-${deployment.id}`} className="rounded-lg border border-line bg-panel-soft p-3">
                  ...
                  <p className="mt-3 truncate font-mono text-xs text-slate-300">{deployment.version ?? deployment.commitSha ?? "unversioned"}</p>
                </div>
              ))}
              {latestByService.length === 0 ? <p className="text-sm text-slate-500">No service release state is available yet.</p> : null}
            </div>
          </Card>
          ...
          <Card title="Release Risk" description="Open incidents on services with current release records.">
            <div className="grid gap-2">
              {releaseIncidents.map((incident) => (
                <div key={incident.id} className="flex items-center justify-between gap-3 rounded-lg border border-line bg-panel-soft p-3">
                  <div className="min-w-0">
                    <p className="truncate text-sm font-semibold text-white">{incident.title}</p>
                    <p className="mt-1 text-xs text-slate-500">{incident.status}</p>
                  </div>
                  <ShieldAlert className="h-4 w-4 text-rose" />
                </div>
              ))}
              {releaseIncidents.length === 0 ? <p className="text-sm text-slate-500">No open incidents are associated with current releases.</p> : null}
            </div>
          </Card>
        </div>
      </div>
