    ...
    <div className="grid gap-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">Live reliability board for {environment} over the active time range.</p>
        </div>
      ...
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <StatCard label="Logs ingested" value={numberValue(summary.logsIngested ?? summary.logs)} detail={`${numberValue(summary.metricsIngested ?? summary.metrics)} metrics`} />
      </div>

      <div className="grid gap-5 xl:grid-cols-[1.2fr_0.8fr]">
        <Card title="Error Trend" description="Bucketed errors from backend telemetry">
          <div className="flex h-56 items-end gap-2">
            {trends.length === 0 ? <p className="text-sm text-slate-500">No error buckets returned for the selected range.</p> : null}
            {trends.slice(-24).map((bucket, index) => {
              ...
              return (
                <div key={`${bucket.bucket ?? index}`} className="flex min-w-0 flex-1 flex-col items-center gap-2">
                  <div className="w-full rounded-t bg-rose/70" style={{ height: `${Math.max(4, (value / maxTrend) * 180)}px` }} title={`${value} errors`} />
                  <span className="w-full truncate text-center text-[10px] text-slate-600">{String(bucket.bucket ?? index).slice(-5)}</span>
                </div>
              );
            })}
          </div>
        ...
        <Card title="Service Health" description="Current health states for selected environment">
          ...
          <div className="mt-4 grid gap-2">
            {services.slice(0, 6).map((service) => (
              <div key={service.id} className="flex items-center justify-between gap-3 rounded-md border border-line bg-panel-soft px-3 py-2">
        ...
        <Card title="Open Issues" description="Incidents requiring attention">
          <div className="grid gap-2">
            {activeIncidents.slice(0, 6).map((incident) => (
              <div key={incident.id} className="rounded-md border border-line bg-panel-soft p-3">
        ...
        <Card title="Release Activity" description="Recent deployment records">
          <div className="grid gap-2">
            {deployments.slice(0, 6).map((deployment) => (
              <div key={deployment.id} className="rounded-md border border-line bg-panel-soft p-3">
                <p className="truncate text-sm font-semibold text-white">{deployment.serviceName}</p>
                <p className="mt-1 text-xs text-slate-500">{deployment.version ?? deployment.commitSha ?? "unversioned"} / {deployment.status ?? "received"}</p>
              </div>
            ))}
        ...
        <Card title="Reports" description="Generated reliability reports">
          <div className="grid gap-2">
            {reports.slice(0, 6).map((report) => (
              <div key={report.id} className="rounded-md border border-line bg-panel-soft p-3">
                <div className="flex items-center gap-2">
                  <BarChart3 className="h-4 w-4 text-mint" />
                  <p className="truncate text-sm font-semibold text-white">{report.title}</p>
                </div>
                <p className="mt-1 text-xs text-slate-500">{report.reportType} / {report.status}</p>
              </div>
            ))}
...
function Empty({ icon, text }: { icon: ReactNode; text: string }) {
  return (
    <div className="flex items-center gap-2 rounded-md border border-line bg-panel-soft px-3 py-2 text-sm text-slate-500">
      {icon}
      {text}
    </div>
  );
