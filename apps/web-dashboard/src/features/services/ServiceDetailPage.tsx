...
export function ServiceDetailPage({ serviceId, onBack }: ServiceDetailPageProps) {
  ...
  async function loadData() {
    ...
    try {
      ...
      const [currentProject, allIncidents, allDeployments, allLogs, allRules, performance, allMetrics, detailSummary, aggregates] = await Promise.all([
  ...
  const isQueue = normalizedType === "queue" || normalizedType === "message-broker" || serviceName.includes("rabbit") || serviceName.includes("kafka");
  ...
  const trendData = useMemo(() => {
    return [...latencyAggregates].reverse().slice(-12).map((row) => ({
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">
              Project: <span className="font-semibold text-slate-300">{project?.name || "Loading..."}</span> · Env:{" "}
              <span className="font-semibold text-slate-300">{service?.environment}</span>
            </p>
            <p className="mt-1 text-xs text-slate-500">
              Health: <span className="font-semibold capitalize text-slate-300">{service?.healthStatus ?? "unknown"}</span>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-lg border border-line bg-panel p-5 shadow-panel">
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Throughput</p>
        ...
        <div className="rounded-lg border border-line bg-panel p-5 shadow-panel">
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Latency (p50/p95/p99)</p>
          <p className="mt-2 text-2xl font-bold text-white">
            {numberFmt(summary.p50LatencyMs, 0)} <span className="text-sm font-normal text-slate-400">/</span> {numberFmt(summary.p95LatencyMs, 0)} <span className="text-sm font-normal text-slate-400">/</span> {numberFmt(summary.p99LatencyMs, 0)} <span className="text-xs font-normal text-slate-500">ms</span>
          </p>
          <p className="mt-1 text-xs text-slate-500">Percentile buckets</p>
        </div>

        <div className="rounded-lg border border-line bg-panel p-5 shadow-panel">
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Error Rate</p>
        ...
        <div className="rounded-lg border border-line bg-panel p-5 shadow-panel">
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Uptime</p>
          <p className="mt-2 text-2xl font-bold text-white">{numberFmt(summary.uptimePercent, 2)}%</p>
          <p className="mt-1 text-xs text-slate-500">Calculated over {timeRange}</p>
        </div>
        ...
        <div className="rounded-lg border border-line bg-panel p-5 shadow-panel">
          ...
          <p className="mt-1 text-xs text-slate-500">Open service incidents</p>
        </div>

        <div className="rounded-lg border border-line bg-panel p-5 shadow-panel">
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Log Volume</p>
      ...
      {isCache && (
        <div className="grid gap-4 md:grid-cols-4 rounded-lg border border-indigo-500/20 bg-indigo-500/5 p-5">
          ...
          <div>
            <p className="text-xs text-slate-400 font-semibold uppercase">Cache Hit Ratio</p>
            <p className="text-xl font-bold text-white mt-1">{numberFmt(metricValue(["redis_cache_hit_ratio"]), 2)}%</p>
          </div>
          <div>
            <p className="text-xs text-slate-400 font-semibold uppercase">Connected Clients</p>
            <p className="text-xl font-bold text-white mt-1">{numberFmt(metricValue(["redis_connected_clients"], 0), 0)} active</p>
          </div>
          <div>
            <p className="text-xs text-slate-400 font-semibold uppercase">Memory Used</p>
            <p className="text-xl font-bold text-white mt-1">{numberFmt(metricValue(["redis_memory_used_bytes"], 0) / 1024 / 1024, 1)} MB</p>
          </div>
          <div>
            <p className="text-xs text-slate-400 font-semibold uppercase">Evicted Keys</p>
      ...
      {isDb && (
        <div className="grid gap-4 md:grid-cols-4 rounded-lg border border-mint/20 bg-mint/5 p-5">
          <div className="md:col-span-4 flex items-center gap-2 border-b border-mint/10 pb-2">
          ...
          <div>
            <p className="text-xs text-slate-400 font-semibold uppercase">Active Connections</p>
            <p className="text-xl font-bold text-white mt-1">{numberFmt(metricValue(["postgres_connections_active"], 0), 0)} / {numberFmt(metricValue(["postgres_connections_max"], 0), 0)} max</p>
          </div>
          <div>
            <p className="text-xs text-slate-400 font-semibold uppercase">Avg DB Query Latency</p>
            <p className="text-xl font-bold text-white mt-1 text-amber">{numberFmt(metricValue(["postgres_query_duration_ms", "postgres_query_latency_ms"], 0), 1)} ms</p>
          </div>
          <div>
            <p className="text-xs text-slate-400 font-semibold uppercase">Buffer Cache Hit Ratio</p>
            <p className="text-xl font-bold text-white mt-1">{numberFmt(metricValue(["postgres_cache_hit_ratio"], 0), 2)}%</p>
          </div>
          <div>
            <p className="text-xs text-slate-400 font-semibold uppercase">Deadlocks</p>
            <p className="text-xl font-bold text-white mt-1 text-emerald-400">{numberFmt(metricValue(["postgres_deadlocks_total"], 0), 0)}</p>
          </div>
        </div>
      )}

      {isQueue && (
        <div className="grid gap-4 md:grid-cols-4 rounded-lg border border-amber/20 bg-amber/5 p-5">
          ...
          <div>
            <p className="text-xs text-slate-400 font-semibold uppercase">Queue Depth</p>
            <p className="text-xl font-bold text-white mt-1">
              {numberFmt(metricValue(["rabbitmq_queue_depth", "kafka_consumer_lag"], 0), 0)} pending
            </p>
          </div>
          <div>
            <p className="text-xs text-slate-400 font-semibold uppercase">Messages In</p>
            <p className="text-xl font-bold text-white mt-1">
              {numberFmt(metricValue(["kafka_messages_in_total", "rabbitmq_messages_published_total"], 0), 0)}
            </p>
          </div>
          <div>
            <p className="text-xs text-slate-400 font-semibold uppercase">Consumer Lag</p>
            <p className="text-xl font-bold text-white mt-1 text-emerald-400">{numberFmt(metricValue(["kafka_consumer_lag"], 0), 0)}</p>
          </div>
          <div>
            <p className="text-xs text-slate-400 font-semibold uppercase">Failed Messages</p>
            <p className="text-xl font-bold text-white mt-1 text-emerald-400">{numberFmt(metricValue(["rabbitmq_messages_failed_total"], 0), 0)}</p>
          </div>
        </div>
      )}
            ...
            {trendData.map((d, i) => (
              <div key={i} className="flex h-full flex-1 flex-col justify-end items-center group relative">
                ...
                <div className="w-full flex flex-col justify-end h-full">
                  <div
                    style={{ height: `${(d.p99 / maxLatency) * 100}%` }}
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="rounded-lg border border-line bg-panel p-5 shadow-panel">
            ...
            <div className="max-h-60 overflow-y-auto rounded bg-[#070b0e] p-3 font-mono text-xs text-slate-300">
              {logs.map((log, index) => (
