    ...
    <div className="grid gap-4 xl:grid-cols-[420px_1fr]">
      <form onSubmit={submit} className="rounded-lg border border-line bg-panel p-4 shadow-panel">
        ...
        <div className="grid gap-3">
          <input className="h-10 rounded-md border border-line bg-[#0d1419] px-3 text-sm" value={organizationId} onChange={(event) => setOrganizationId(event.target.value)} />
          <select className="h-10 rounded-md border border-line bg-[#0d1419] px-3 text-sm" value={provider} onChange={(event) => setProvider(event.target.value as "email" | "slack" | "discord")}>
            <option value="email">Email</option>
            <option value="slack">Slack</option>
            <option value="discord">Discord</option>
          </select>
          <input className="h-10 rounded-md border border-line bg-[#0d1419] px-3 text-sm" value={destination} onChange={(event) => setDestination(event.target.value)} />
      ...
      <section className="rounded-lg border border-line bg-panel p-4 shadow-panel">
        ...
        <div className="grid gap-3">
          {history.map((entry) => (
            <div key={entry.id} className="rounded-lg border border-line bg-[#0d1419] p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="text-sm font-medium text-white">{entry.subject}</p>
                <span className="rounded-md bg-[#152129] px-2 py-1 text-xs text-slate-300">{entry.provider}</span>
              </div>
