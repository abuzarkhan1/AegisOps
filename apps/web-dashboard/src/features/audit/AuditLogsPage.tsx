...
const actionTone = (action: string) => {
  if (action.includes("created") || action.includes("generated")) return "border-white/20 bg-white/10 text-white";
  if (action.includes("deleted") || action.includes("revoked") || action.includes("resolved"))
    return "border-rose-500/30 bg-rose-500/10 text-rose-300";
  if (action.includes("updated") || action.includes("assigned") || action.includes("acknowledged"))
    return "border-amber-500/30 bg-amber-500/10 text-amber-300";
  return "border-slate-600 bg-white/10 text-text-soft";
};
      ...
      <aside className="space-y-4">
        <form onSubmit={submit} className="aegis-glass rounded-2xl p-5 shadow-panel">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-text-soft">Enterprise action trail</p>
            </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="aegis-glass rounded-2xl p-3 shadow-panel">
            <p className="text-xs uppercase text-text-soft">Events</p>
            <p className="mt-1 text-xl font-bold text-white">{logs.length}</p>
          </div>
          <div className="aegis-glass rounded-2xl p-3 shadow-panel">
            <p className="text-xs uppercase text-text-soft">Actions</p>
            <p className="mt-1 text-xl font-bold text-white">{actionCount}</p>
          </div>
          <div className="aegis-glass rounded-2xl p-3 shadow-panel">
            <p className="text-xs uppercase text-text-soft">Actors</p>
        ...
        <section className="aegis-glass rounded-2xl p-5 shadow-panel">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-text-soft">Newest recorded actions first</p>
            </div>
            <button
              type="button"
              onClick={() => load()}
              ...
              <tbody className="divide-y divide-line/40">
                {logs.map((log) => (
                  <tr
                    ...
                    className={`cursor-pointer transition hover:bg-white/5/60 ${selectedLog?.id === log.id ? "bg-white/5" : ""}`}
                  >
                    <td className="py-3 pr-4 text-xs text-text-soft">{formatDate(log.createdAt)}</td>
                    <td className="py-3 pr-4">
                      <span className={`rounded-full border px-2 py-1 text-xs font-semibold ${actionTone(log.action)}`}>{log.action}</span>
                    </td>
                    <td className="py-3 pr-4 text-text-soft">
                      <div className="font-medium text-white">{log.resourceType}</div>
                      <div className="font-mono text-xs text-text-muted">{compactId(log.resourceId)}</div>
                    </td>
                    <td className="py-3 pr-4 font-mono text-xs text-text-soft">{compactId(log.actorId)}</td>
                    <td className="py-3">
                      <span className="rounded-2xl border border-white/10 bg-white/5 px-2 py-1 text-xs text-text-soft">
                        {log.status ?? "recorded"}
                      </span>
                    </td>
                  </tr>
          ...
          {selectedLog ? (
            <div className="grid gap-4 lg:grid-cols-[280px_1fr]">
              <div className="space-y-3 text-sm">
                <p>
                  <span className="block text-xs uppercase text-text-muted">Audit ID</span>
                  <span className="font-mono text-text-primary">{selectedLog.id}</span>
                </p>
                <p>
                  <span className="block text-xs uppercase text-text-muted">Organization</span>
                  <span className="font-mono text-text-primary">{selectedLog.organizationId ?? "none"}</span>
                </p>
                <p>
                  <span className="block text-xs uppercase text-text-muted">Actor</span>
                  <span className="font-mono text-text-primary">{selectedLog.actorId ?? "system"}</span>
                </p>
                <p>
                  <span className="block text-xs uppercase text-text-muted">IP Address</span>
                  <span className="font-mono text-text-primary">{selectedLog.ipAddress ?? "not captured"}</span>
                </p>
              </div>
            ...
            <div className="flex items-center gap-2 rounded-2xl border border-dashed border-white/10 p-4 text-sm text-text-soft">
