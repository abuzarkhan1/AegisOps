    ...
    <div className="grid gap-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-text-soft">Organization profiles and runtime configuration.</p>
        </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[360px_1fr]">
        <form onSubmit={submitCreate}>
          <Card
            ...
            description="Add a workspace record through the core API."
            action={<Building2 className="h-5 w-5 text-white" />}
          >
        ...
        <Card title="Organizations" description="Update organization name and plan.">
          <div className="grid gap-3 md:grid-cols-2">
            {organizations.map((organization) => (
              <form key={organization.id} onSubmit={(event) => submitUpdate(event, organization)} className="aegis-glass rounded-2xl p-4">
                <p className="truncate text-sm font-semibold text-white">{organization.slug}</p>
                <p className="mt-1 truncate text-xs text-text-muted">{organization.id}</p>
            ...
            {organizations.length === 0 ? <p className="text-sm text-text-muted">No organizations found.</p> : null}
          </div>
        </Card>
      </div>
    </div>
