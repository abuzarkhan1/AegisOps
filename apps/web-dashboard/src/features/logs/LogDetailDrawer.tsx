...
export function LogDetailDrawer({ log, onClose }: LogDetailDrawerProps) {
  return (
    <aside className="w-full rounded-lg border border-line bg-panel p-6 shadow-panel lg:w-96">
      ...
      <div className="space-y-4">
        ...
        <Detail label="Service Name" strong>{log.serviceName}</Detail>
        <Detail label="Environment">{log.environment}</Detail>
        <div>
          <p className="text-[10px] font-bold uppercase text-slate-500">Level</p>
          <div className="mt-1"><LogLevelBadge level={log.level} /></div>
        </div>
        <Detail label="Trace ID" mono>{log.traceId || "-"}</Detail>
        <Detail label="Request ID" mono>{log.requestId || "-"}</Detail>
        <div className="grid grid-cols-3 gap-2">
          <Detail label="Route" mono>{log.route || log.metadata?.route || "-"}</Detail>
          <Detail label="Status" mono>{log.statusCode || log.metadata?.statusCode || "-"}</Detail>
          <Detail label="Duration" mono>{log.durationMs || log.metadata?.durationMs || "-"}ms</Detail>
        </div>
        <div>
        ...
        <div>
          <p className="mb-1 text-[10px] font-bold uppercase text-slate-500">Metadata JSON</p>
